...
* Webフォームからの手入力による計測データ送信
  * 物件名・複数部屋（物件に合わせて追加／削除可能）・各部屋の計測点（床 X/Y/斜め・壁 下/左・柱 下/右）の入力に対応。
  * 計測点ごとに「方向・判定」「測定値の差」「距離」「水平器計測値」を記入でき、換算計測値はExcel側の数式で自動計算されます。
...
| `room_block.fields` | 計測点1つあたりの記入列（`direction`=方向, `digital_level`=水平器計測値, `diff`=測定値の差, `distance`=距離） |
| `room_block.measurements` | 1部屋内の計測点と、ブロック先頭行からの相対行 `row_offset` |
| `block_start_rows` | 各部屋ブロックの先頭行（**この要素数＝対応できる部屋数**） |

部屋 `i`（0始まり）の各セルは「`block_start_rows[i]` ＋ `row_offset`」で求まります。印字済みのラベル・区切りの「/」・換算計測値の数式などは記入欄ではないため `mapping.json` には含めず、書き込み・クリアの対象外です。
...
> フロントエンド（`gas/index.html`）の計測点定義 `MEASUREMENT_GROUPS` の `key` は、`mapping.json` の `measurements[].key` と一致させる必要があります。計測点を増減する際は両方を更新してください。
