...
* Webフォームからの手入力による計測データ送信
  ...
  * **新しい部屋を追加すると、階数には直前の部屋の階数が初期値として入ります**（最初の部屋は 1 階）。同じ階の部屋を続けて入力するときの手間を減らします。
  * **出力前に簡易バリデーションを行い、入力ミスの可能性があれば確認ダイアログを表示します**（出力はブロックせず、確認後そのまま出力可）。ルールは `functions/mapping.json` の `validation` に外出ししています（後述）。
...
| `validation` | 出力前の簡易バリデーション設定（しきい値・例外の選択肢・必須の選択欄）。フロントエンドの `VALIDATION` 定数と一致させる |
...
### 出力前の簡易バリデーション (`validation`)

「Excel出力」ボタンを押すと、フロントエンド（`gas/index.html`）が出力前に入力ミスの可能性をチェックし、該当があれば確認ダイアログを表示します。**入力をブロックする厳格チェックではなく確認のための警告**で、確認後はそのまま出力できます（階数・部屋名の未入力だけは従来どおり出力を止めます）。

判定の組み合わせ（しきい値・例外の選択肢・必須の選択欄）は運用で変わりうるため、ソースに直書きせず `functions/mapping.json` の `validation` に外出ししています。

| キー | 意味 |
| --- | --- |
| `slope_warning_threshold` | 傾斜の警告しきい値（1000分率）。水平器計測値、または `1000×差÷距離` の絶対値がこの値以上だと再検査になりうるため確認を促す（既定 `6`） |
| `no_value_select_options` | 選択しても計測値の入力が不要な選択肢（`傾斜無`・`―`）。これら以外を選んでいるのに数値が未入力なら確認を促す |
| `require_select_keys` | 必ず選択させる計測点の `key`（柱は必ず選択するため未選択を確認） |

> この設定はフロントエンド（`gas/index.html`）の `VALIDATION` 定数でバリデーションに使います。`MEASUREMENT_GROUPS` と同様、`mapping.json` の `validation` と `VALIDATION` の値は一致させてください（バックエンドはこのキーを使いません）。

### フォーマット改訂時の読み取り・マッピング更新手順（重要）

将来フォーマットが差し替えられた際、見た目だけで判断すると解釈を誤ります（実際に過去、見た目のラベルから「壁・柱も傾斜方向を入力する」と誤読しました）。新しい雛形を読み直すときは、必ず以下の **客観的な情報源** を上から順に確認し、推測でマッピングを書かないでください。

1. **データ入力規則（プルダウン）を最優先の正とする。** どのセルに何を入力するか・選択肢は何かは、見た目のラベルではなく **セルに設定されたプルダウン（data validation）** で決まります。プルダウンが付いているセルが「記入欄」、付いていない文字は「印字済みラベル」と判断します。
   * 例として `傾斜測定` シートでは、床の傾斜方向は **S 列** に、壁・柱の「測定した向き」は **P 列** にプルダウンが設定されています。床の P 列（`X方向` 等）はプルダウンの無い印字済みラベルなので書き込みません。
2. **結合セル（merged cells）でブロックの繰り返し構造と記入欄の左上アンカーを把握する。** 値は結合範囲の左上セルに書き込みます。繰り返しの先頭行の並びが `block_start_rows`、ブロック内の相対行が `row_offset` になります。
3. **数式・区切り文字・分母などの固定値は記入欄ではない。** 換算計測値（`=IFERROR(...)`）・区切りの「/」・分母 `1000` などは雛形側の固定要素なので、マッピングに含めず書き込み・クリアの対象にしません。

#### 抽出に使えるスニペット
`openpyxl` で以下を出力し、その結果（特に **データ入力規則**）に基づいて `mapping.json` を更新します。
...
#### マッピング・フォーム反映時の原則
* `select.col` / `select.options` は **プルダウンの列・選択肢をそのまま**写します（並び順・記号も雛形通り）。**勝手に選択肢を足さない**でください（例: 壁のプルダウンに `―` は無いので追加しない。柱だけに `―` がある、といった差異もそのまま再現します）。
* 選択値（`←` や `―`、`上壁` など）は数値ではないため、バックエンドでは数値化・正規化せず **文字列のまま**書き込み、プルダウン候補と完全一致させます（`―` は U+2015。全角ダッシュ等に置換しないこと）。
* 変更は `functions/mapping.json`（記入位置・選択肢）と `gas/index.html` の `MEASUREMENT_GROUPS`（フォーム表示・同じ選択肢）の **両方** を更新し、`functions/test_main.py` の期待値も合わせます。
